...
/* Procedure to add all reserved seats prices to a total gross profit */
DROP PROCEDURE IF EXISTS total_gross_profit;
DELIMITER //
CREATE PROCEDURE total_gross_profit()
BEGIN
  SELECT SUM(section.price) AS 'Total Gross Profit'
    FROM reservation NATURAL JOIN flight_seat_availability NATURAL JOIN seat NATURAL JOIN section
