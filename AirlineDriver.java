...
public class AirlineDriver {
	...
	public static void printFlightInfo(ArrayList<Flight>f) {
		...
		boolean more=true;
		while (more) {
			System.out.println("Please select the numerical value for the flight that you would like to print information on:");
			for (int i=0; i<f.size(); i++) {
	...
	public static void bookReservation(ArrayList<Customer> c, ArrayList<Pilot> p, ArrayList<Reservation> r, ArrayList<Flight> f) {
		...
		while (more) {
			...
			//for a reservation happening at the end after the total loop is done
				...
				//booking reservation
				System.out.println("Booked reservation for: " + c.get(custNum).getfirstName()+ " "+c.get(custNum).getlastName()+ " on flight " + f.get(flightnum).getFlightNum() + " flying "+ f.get(flightnum).getRoute()+ " on " + f.get(flightnum).getDate()+ " at "+ f.get(flightnum).getTime()+ " with " + partyNum + " seat(s) for a total cost of "+ nf.format(cost) +".");
	...
	public static void printGrossIncome(ArrayList<Flight> f) {
		...
		while (more) {
			...
			System.out.println("Which is the flight you want to print out gross income for?");
			for (int i =0; i<f.size();i++) {
